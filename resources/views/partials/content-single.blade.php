...
<div class="bg-slate-50 min-h-screen pb-20 font-sans antialiased text-slate-800 pt-6">
    <style>
        ...
        .prose figure,
        .e-content figure,
        .dailyve-single figure,
        .prose .wp-block-image,
        .e-content .wp-block-image,
        .dailyve-single .wp-block-image {
            max-width: 100% !important;
            width: 100% !important;
    ...
    <div class="mx-auto grid max-w-7xl gap-8 px-4 pt-4 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 items-start">
        ...
        <div
            class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm shadow-slate-100/50 max-w-full overflow-hidden">
            <article class="{{ implode(' ', get_post_class('dailyve-single')) }}">
                ...
                <header class="mb-8 pb-6 border-b border-slate-100">
                    ...
                    <!-- Post Title -->
                    <h1 class="font-display font-semibold text-slate-900 tracking-tight leading-tight mb-6">
                        {!! html_entity_decode(get_the_title(), ENT_QUOTES, 'UTF-8') !!}
                    </h1>

                    <!-- Entry Meta Info -->
                    <div
                        class="flex flex-wrap items-center text-xs md:text-sm font-semibold text-slate-400 gap-4 md:gap-6">
                ...
                @if (has_post_thumbnail())
                    <div
                        class="relative aspect-[16/9] w-full overflow-hidden rounded-2xl border border-slate-100 shadow-sm mb-8 bg-slate-50">
                ...
                <div
                    class="mt-12 pt-6 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <!-- Post Tags -->
                        ...
                        @if ($tags)
                            @foreach ($tags as $tag)
                                <a href="{{ esc_url(get_tag_link($tag->term_id)) }}"
                                    class="text-xs font-semibold text-slate-500 bg-slate-100 hover:bg-slate-200 px-3 py-1 rounded-xl transition duration-150">
                                    #{{ $tag->name }}
                                </a>
                            @endforeach
                        @endif
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <!-- Copy Link -->
                        <button
                            onclick="navigator.clipboard.writeText('{{ get_permalink() }}'); alert('Đã sao chép liên kết vào bộ nhớ tạm!');"
                ...
                @endphp
                <div
                    class="mt-12 bg-slate-50 border border-slate-100 rounded-2xl p-6 flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-6 shadow-sm">
                    ...
                    <div class="flex-grow">
                        ...
                        <p class="text-xs text-slate-500 leading-relaxed font-medium">
                            {!! $author_desc ?:
                                'Chuyên viên biên tập nội dung cẩm nang xe khách, tin tức ưu đãi và chia sẻ trải nghiệm du lịch tại Dailyve.' !!}
                        </p>
                    </div>
                </div>
            ...
            <div class="relative group">
                <div id="single-related-slider" class="flex overflow-x-auto gap-6 scrollbar-none pb-4 scroll-smooth">
                    @while ($related_posts->have_posts())
                        ...
                        <a href="{{ $post_url }}"
                            class="group flex-shrink-0 w-80 bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-blue-100/50 transition-all duration-300 overflow-hidden flex flex-col">
                            <div class="relative aspect-[16/10] overflow-hidden bg-slate-100">
                                ...
                                @else
                                    <div
                                        class="w-full h-full flex items-center justify-center text-blue-500 bg-blue-50">
                            ...
                            <div class="p-5 flex-grow flex flex-col justify-between bg-white">
                                <h4
                                    class="text-sm font-semibold text-slate-800 line-clamp-2 leading-snug group-hover:text-blue-600 transition-colors">
                                    {!! $post_title !!}
                                </h4>
                                <div
                                    class="flex items-center gap-3 text-[10px] font-semibold text-slate-400 mt-4 pt-3 border-t border-slate-50">
                ...
                <button
                    onclick="document.getElementById('single-related-slider').scrollBy({ left: -320, behavior: 'smooth' })"
