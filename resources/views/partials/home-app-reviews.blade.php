<section class="relative overflow-hidden bg-[#03162f] text-white" aria-labelledby="dailyve-reviews-title"
    ...
    <div class="dailyve-container relative py-12 sm:py-16 lg:py-20">
        <div class="mx-auto max-w-5xl text-center">
            ...
            <h2 id="dailyve-reviews-title" class="m-0 text-[28px] font-bold leading-tight text-white sm:text-4xl lg:text-5xl">
            ...
            <div class="mt-4 flex flex-wrap items-center justify-center gap-2 sm:gap-3 text-slate-300">
                <strong class="text-[32px] font-bold text-[#3b82f6] sm:text-4xl leading-none">4.8<span class="text-white text-xl sm:text-3xl font-medium">/5</span></strong>
                    ...
                    <article class="dailyve-testimonial-card min-w-[85%] sm:min-w-[340px] lg:min-w-[calc((100%_-_60px)/4)] snap-start rounded-2xl border border-[#2ea2ff]/30 bg-[#05214a]/75 p-5 shadow-[0_20px_55px_rgba(0,0,0,0.22)] backdrop-blur transition hover:border-[#2ea2ff]/60">
                        <div class="flex items-start gap-4">
                            @if(!empty($review['avatar']))
                                <img src="{{ esc_url($review['avatar']) }}" alt="{{ esc_attr($review['name']) }}" class="h-14 w-14 shrink-0 rounded-full object-cover shadow-inner border-2 border-white/10">
                            @endif
                            <div class="min-w-0 flex-1 text-left">
                                <div class="flex items-start justify-between gap-3">
                                    <h3 class="m-0 truncate text-base font-bold text-white">{{ $review['name'] }}</h3>
                                    <i class="fas fa-quote-right shrink-0 text-xl text-[#1677ff]/80" aria-hidden="true"></i>
                                </div>
                                <div class="mt-1.5 flex gap-0.5 text-xs text-[#ffc107]" aria-label="{{ (int) $review['rating'] }} sao">
        ...
        <div class="mt-8 rounded-2xl border border-[#2ea2ff]/30 bg-[#06224c]/75 px-5 py-6 shadow-[0_18px_55px_rgba(0,0,0,0.2)] backdrop-blur sm:px-8 lg:px-14">
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 sm:gap-6">
                @foreach ($reviewStats as $stat)
                    <div class="flex items-center justify-start sm:justify-center lg:justify-start gap-4 lg:border-r lg:border-[#2ea2ff]/20 lg:pr-8 last:lg:border-r-0 pl-2 sm:pl-0">
                        <span class="flex h-12 w-12 sm:h-14 sm:w-14 shrink-0 items-center justify-center rounded-full border border-[#2ea2ff]/40 bg-[#0a3b78] text-lg sm:text-xl text-[#3b82f6]">
                            <i class="{{ esc_attr($stat['icon']) }}" aria-hidden="true"></i>
                        </span>
                        <div class="text-left">
                            <strong class="block text-lg font-bold text-white sm:text-2xl leading-none">{{ $stat['value'] }}</strong>
                            <span class="mt-1 block text-[12px] sm:text-[13px] text-slate-300">{{ $stat['label'] }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <div class="mt-14 grid items-end gap-9 border-t border-[#2ea2ff]/20 pt-12 lg:grid-cols-[1fr_360px_1fr] lg:gap-12">
            <div class="order-1 lg:pb-8">
                <h2 class="m-0 text-3xl font-medium leading-tight text-white sm:text-4xl">
                    Tải app <strong class="text-[#3b82f6] font-bold">Dailyve</strong> dễ dàng
                </h2>
                <p class="mt-4 max-w-sm text-base leading-7 text-slate-300">
                ...
                <div class="mt-8 grid max-w-sm gap-4">
                    <a href="https://apps.apple.com/vn/app/dailyve-%C4%91%E1%BA%B7t-v%C3%A9-xe-24-7/id6748101538"
                        ...
                        <i class="fas fa-chevron-right text-lg text-slate-400 transition group-hover:translate-x-1" aria-hidden="true"></i>
                    </a>
            ...
            <div class="order-2 flex justify-center relative" aria-hidden="true">
                ...
                <div class="relative h-[420px] w-[220px] overflow-hidden rounded-[2.2rem] border-[8px] border-slate-950 bg-white shadow-[0_0_0_2px_rgba(255,255,255,0.3),0_28px_80px_rgba(0,122,255,0.3)] sm:h-[480px] sm:w-[250px]">
                    <span class="absolute left-1/2 top-0 z-10 h-5 w-24 -translate-x-1/2 rounded-b-xl bg-slate-950"></span>
                    <img src="{{ esc_url($screenHome) }}" alt="" class="h-full w-full object-cover" loading="lazy" decoding="async">
                </div>
            </div>

            <div class="order-3 lg:pb-8">
                <div class="mx-auto max-w-sm rounded-[1.5rem] border border-[#2ea2ff]/40 bg-[#062a5e]/80 p-7 text-center shadow-[0_0_45px_rgba(0,122,255,0.15)] backdrop-blur">
                    ...
                    <h3 class="m-0 mt-5 text-lg font-medium text-white leading-relaxed">Hoặc quét mã QR<br>để tải app Dailyve</h3>
                    <div class="mt-4 flex items-center justify-center gap-4 text-sm text-slate-300">
                        <span class="inline-flex items-center gap-1.5"><i class="fab fa-apple text-white" aria-hidden="true"></i> App Store</span>
                        <span class="inline-flex items-center gap-1.5"><i class="fab fa-google-play text-emerald-400" aria-hidden="true"></i> Google Play</span>
                    </div>
                    <div class="mt-6 grid grid-cols-3 gap-3 border-t border-[#2ea2ff]/20 pt-5">
                        @foreach ($appBenefits as $benefit)
                            <div class="text-center">
                                <span class="mx-auto flex h-10 w-10 items-center justify-center rounded-xl bg-[#0b3d80] text-base text-[#3b82f6]">
                                    <i class="{{ esc_attr($benefit['icon']) }}" aria-hidden="true"></i>
                                </span>
                                <span class="mt-2 block text-[11px] font-medium leading-4 text-slate-200">{{ $benefit['label'] }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
