    ...
    <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm shadow-slate-100/50">
        ...
        <div class="space-y-4">
            @foreach ($services as $service)
                <a href="{{ $service['url'] }}"
                    class="group flex items-start gap-4 p-3 rounded-2xl border border-slate-50 hover:border-blue-100/50 hover:bg-blue-50/10 transition-all duration-300">
                    <div
                        class="flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center border transition-all duration-300 group-hover:scale-105 {{ $service['color'] }}">
                        {!! $service['icon'] !!}
                    </div>
                    <div class="flex-grow">
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm shadow-slate-100/50">
            ...
            <div class="space-y-4">
                @while ($sidebar_posts->have_posts())
                    ...
                    @endphp
                    <a href="{{ $post_url }}"
                        class="group flex gap-4 p-2 rounded-2xl hover:bg-slate-50 transition-all duration-300">
                        @if ($has_thumb)
                            <div
                                class="flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden bg-slate-100 border border-slate-100">
                        ...
                        @else
                            <div
                                class="flex-shrink-0 w-16 h-16 rounded-lg bg-blue-50 border border-blue-50/50 flex items-center justify-center text-blue-500">
                        ...
                        <div class="flex-grow min-w-0 flex flex-col justify-center">
                            <h4
                                class="text-xs font-semibold text-slate-800 line-clamp-2 leading-snug group-hover:text-blue-600 transition-colors">
                                {!! $post_title !!}
                            </h4>
